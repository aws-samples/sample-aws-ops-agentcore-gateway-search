...
def create_agentcore_role(agent_name):
    ...
    try:
        ...
        time.sleep(10)  # INTENTIONAL: AWS IAM role creation requires propagation time
...
def create_agentcore_gateway_role(gateway_name):
    ...
    try:
        ...
        # Pause to make sure role is created
        time.sleep(10)  # INTENTIONAL: AWS IAM gateway role creation requires propagation time
    except iam_client.exceptions.EntityAlreadyExistsException:
        print("Role already exists -- deleting and creating it again")
        policies = iam_client.list_role_policies(
            RoleName=agentcore_gateway_role_name,
...
def create_agentcore_gateway_role_s3_smithy(gateway_name):
    ...
    try:
        ...
        print(f"Role '{agentcore_gateway_role_name}' already exists")
        ...
        print(f"Updated policy for existing role {agentcore_gateway_role_name}")
        
        return existing_role
        
    except iam_client.exceptions.NoSuchEntityException:
        # Role doesn't exist, create it
        print(f"Creating new role: {agentcore_gateway_role_name}")
        agentcore_iam_role = iam_client.create_role(
            RoleName=agentcore_gateway_role_name,
            AssumeRolePolicyDocument=assume_role_policy_document_json
        )

        time.sleep(10)  # INTENTIONAL: Wait for AWS IAM role propagation
        ...
        print(f"Created and configured role {agentcore_gateway_role_name}")
        
        return agentcore_iam_role
        
    except Exception as e:
        print(f"Error in create_agentcore_gateway_role_s3_smithy: {e}")
        raise
...
def create_gateway_lambda(lambda_function_code_path) -> dict[str, int]:
    ...
    try:
        ...
        time.sleep(100)  # INTENTIONAL: Extended wait for complex IAM role propagation
...
def get_or_create_gateway(gateway_client, gateway_name, role_arn, auth_config):
    """Check if gateway exists, return existing or create new one"""
    try:
        list_response = gateway_client.list_gateways(maxResults=100)
        
        for item in list_response['items']:
            if item.get('name') == gateway_name:
                print(f"Gateway '{gateway_name}' already exists with ID: {item['gatewayId']}")
                
                # Get gateway details to retrieve URL
                gateway_details = gateway_client.get_gateway(gatewayIdentifier=item['gatewayId'])
        ...
        print(f"Error in get_or_create_gateway: {e}")
        raise

def get_or_create_gateway_target(gateway_client, gateway_id, target_name, target_config, credential_config):
    """Check if gateway target exists, return existing or create new one"""
    try:
        list_response = gateway_client.list_gateway_targets(
        ...
        for item in list_response['items']:
            if item.get('name') == target_name:
                print(f"Gateway target '{target_name}' already exists with ID: {item['targetId']}")
                return item['targetId']
        
        print(f"Creating new gateway target: {target_name}")
        ...
        print(f"Error in get_or_create_gateway_target: {e}")
        raise

def get_or_create_s3_bucket(bucket_name, region='us-east-1'):
...
def upload_smithy_schema_if_not_exists(bucket_name, key, smithy_file_path, region='us-east-1'):
    """Upload Smithy schema from local file if it doesn't exist in S3"""
    s3_client = boto3.client('s3', region_name=region)
    
    try:
        # Check if object exists
        s3_client.head_object(Bucket=bucket_name, Key=key)
        print(f"Smithy schema '{key}' already exists in bucket")
        return True
    except s3_client.exceptions.NoSuchKey:
        try:
            print(f"Uploading Smithy schema: {key}")
            
            # Read the local Smithy file
            with open(smithy_file_path, 'r') as f:
            ...
            print(f"Uploaded schema: {key}")
            return True
            
        except FileNotFoundError:
            print(f"Warning: Smithy file not found: {smithy_file_path}")
            return False
        except Exception as e:
            print(f"Error uploading schema {key}: {e}")
            return False
    except Exception as e:
        print(f"Error checking schema {key}: {e}")
        return False

def validate_smithy_files():
    """Validate that required smithy API definition files exist"""
    current_dir = os.path.dirname(os.path.abspath(__file__))
    smithy_dir = os.path.join(current_dir, '..', 'smithy', 'api-definitions')
    
    required_files = [
        's3-apis.json',
        'lambda-apis.json', 
        'eks-apis.json',
        'cloudwatch-logs-apis.json'
    ]
    
    missing_files = []
    for file_name in required_files:
        file_path = os.path.join(smithy_dir, file_name)
        if not os.path.exists(file_path):
            missing_files.append(file_name)
    
    if missing_files:
        print(f"❌ Missing required smithy API definition files: {missing_files}")
        print(f"📥 Please download them from: https://github.com/aws/api-models-aws/tree/main/models")
        print(f"📁 Place them in: {smithy_dir}")
        print(f"Required files:")
        for file_name in missing_files:
            print(f"  - {file_name}")
        return False
    
    print(f"✅ All required smithy API definition files found")
    return True

def setup_smithy_schemas(region='us-east-1'):
    """Setup S3 bucket and Smithy schemas for all services"""
    
    # Validate required files exist first
    if not validate_smithy_files():
        return False
    
    bucket_name = f'agentcore-gateway-search-{region}'
    
    # Create bucket if needed
    if not get_or_create_s3_bucket(bucket_name, region):
        return False
    
    # Get the smithy directory path
    current_dir = os.path.dirname(os.path.abspath(__file__))
    smithy_dir = os.path.join(current_dir, '..', 'smithy', 'api-definitions')
    
    # Service schemas to upload - use existing S3 key names
    services = {
        's3-apis.json': os.path.join(smithy_dir, 's3-apis.json'),
        'lambda-apis.json': os.path.join(smithy_dir, 'lambda-apis.json'),
        'eks-apis.json': os.path.join(smithy_dir, 'eks-apis.json'),
        'cloudwatch-logs-apis.json': os.path.join(smithy_dir, 'cloudwatch-logs-apis.json')
    }
    
    # Upload schemas if they don't exist
    for key, file_path in services.items():
        upload_smithy_schema_if_not_exists(bucket_name, key, file_path, region)
